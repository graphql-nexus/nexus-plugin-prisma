...
import { setupNexusConfig } from '../src/nexusConfig'
...
const resolveBuilderConfig = (config: core.BuilderConfig) => {
  let internalBuilderConfig = { ...config }
  const builder: core.PluginBuilderLens = {
    hasType: throws,
    addType: throws,
    setConfigOption: (k, v) => {
      internalBuilderConfig = { ...internalBuilderConfig, [k]: v }
    },
    hasConfigOption: k => internalBuilderConfig.hasOwnProperty(k),
    getConfigOption: k => internalBuilderConfig[k],
  }
  setupNexusConfig(builder)
  return internalBuilderConfig
}
...
describe('resolveBuilderConfig() outputs', () => {
  ...
  type Cases = [core.BuilderConfig, core.BuilderConfig][]
  const onCases: Cases = [
    // defaults
    [{}, outputs.justTypegen],
...
describe('Environment variable influence over builder config shouldGenerateArtifacts', () => {
  ...
  ])("when defined, overrules environment variables %j", (...args) => {
    const [envEntries, isGenOn] = args
      Object.assign(process.env, envEntries);
      const internalConfig = resolveBuilderConfig(isGenOn);
      const expectedOutputs = isGenOn.shouldGenerateArtifacts ? outputs.default.outputs : outputs.none.outputs;
  ...
  ])("when undefined, NEXUS_SHOULD_GENERATE_ARTIFACTS is used (%j)", (...args) => {
      const [envEntries, isGenOn] = args
      Object.assign(process.env, envEntries);
  ...
  ])("when undefined, and no NEXUS_SHOULD_GENERATE_ARTIFACTS, NODE_ENV is used (%j)", (...args) => {
      const [envEntries, isGenOn] = args
      Object.assign(process.env, envEntries);
      const internalConfig = resolveBuilderConfig({});
      const expectedOutputs = isGenOn ? outputs.default.outputs : outputs.none.outputs;
